...
const fetchSearchResults = async (newestID, resource) => {
  ...
  }q=(verified) (${resourceTypes[resource].join(
    " OR "
  )}) -"request" -"requests" -"requesting" -"needed" -"needs" -"need" -"seeking" -"seek" -"not verified" -"looking" -"unverified" -"urgent" -"urgently" -"urgently required" -"send" -"help" -"get" -"old" -"male" -"female" -"saturation" -is:retweet -is:quote&count=${MAX_RESULTS}&tweet_mode=extended&include_entities=false&expansions=author_id`;
...
const fetchTweets = async () => {
  ...
  await Promise.all(
    Object.keys(resourceTypes).map(async (resource) => {
